// import React from "react";
// import { Link } from "react-router-dom";

// const DownArrow = () => (
//   <div className="text-center text-gray-500 mb-10">
//     <svg
//       className="w-8 h-8 mx-auto animate-bounce"
//       fill="none"
//       stroke="currentColor"
//       viewBox="0 0 24 24"
//       xmlns="http://www.w3.org/2000/svg"
//     >
//       <path
//         strokeLinecap="round"
//         strokeLinejoin="round"
//         strokeWidth="2"
//         d="M19 14l-7 7m0 0l-7-7m7 7V3"
//       ></path>
//     </svg>
//   </div>
// );

// const ServiceCard = React.memo(
//   ({ imageSrc, title, description, features, path }) => (
//     <div className="bg-white rounded-3xl overflow-hidden shadow-lg transition-transform duration-300 ease-in-out lg:mx-8">
//       <div className="p-7">
//         <img
//           src={imageSrc}
//           alt={title}
//           className="w-full h-64 object-cover rounded-xl shadow-lg"
//           loading="lazy"
//         />
//       </div>
//       <div className="p-6">
//         <h3 className="text-2xl font-bold mb-2 text-black">{title}</h3>
//         <p className="text-gray-600">{description}</p>
//         <ul className="mt-4 text-gray-700">
//           {features.map((feature, index) => (
//             <li key={index} className="flex items-center mb-2">
//               <svg
//                 className="w-4 h-4 mr-2 text-custom-blue"
//                 fill="currentColor"
//                 viewBox="0 0 20 20"
//                 xmlns="http://www.w3.org/2000/svg"
//               >
//                 <circle cx="10" cy="10" r="8" />
//               </svg>
//               {feature}
//             </li>
//           ))}
//         </ul>
//         <Link
//           to={path}
//           className="mt-4 relative inline-flex items-center justify-center p-4 px-6 py-3 overflow-hidden font-medium text-indigo-600 transition duration-300 ease-out border-2 border-custom-blue rounded-full shadow-md group"
//         >
//           <span className="absolute inset-0 flex items-center justify-center w-full h-full text-white duration-300 -translate-x-full bg-custom-blue group-hover:translate-x-0 ease">
//             <svg
//               className="w-6 h-6"
//               fill="none"
//               stroke="currentColor"
//               viewBox="0 0 24 24"
//               xmlns="http://www.w3.org/2000/svg"
//             >
//               <path
//                 strokeLinecap="round"
//                 strokeLinejoin="round"
//                 strokeWidth="2"
//                 d="M14 5l7 7m0 0l-7 7m7-7H3"
//               ></path>
//             </svg>
//           </span>
//           <span className="absolute flex items-center justify-center w-full h-full text-custom-blue transition-all duration-300 transform group-hover:translate-x-full ease">
//             Learn More
//           </span>
//           <span className="relative invisible">Learn More</span>
//         </Link>
//       </div>
//     </div>
//   )
// );

// const Services = () => (
//   <div className="bg-gradient-to-b from-custom-blue to-white p-6 lg:p-10">
//     <section id="services" className="container mx-auto my-12">
//       <h1 className="pl-2 text-center text-3xl font-bold border-custom-blue md:text-5xl">
//         Nasze Usługi
//       </h1>
//       <p className="text-lg text-gray-500 text-center p-5">
//         Podnieś standard swojego auta dzięki naszym profesjonalnym usługom detailingu.
//       </p>
//       <DownArrow />
//       <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
//         {[
//           {
//             imageSrc: "/images/Paint Correction.jpg",
//             title: "Korekta Lakieru",
//             description:
//               "Przywróć blask lakieru i nadaj swojemu samochodowi wygląd salonowy dzięki naszym precyzyjnym usługom detailingu.",
//             features: [
//               "1-etapowa i 2-etapowa korekta lakieru",
//               "Usuwanie drobnych i głębokich zarysowań",
//               "Przywrócenie blasku jak w salonie",
//             ],
//             path: "/paint-correction",
//           },
//           {
//             imageSrc: "/images/interior.jpg",
//             title: "Detailing Wnętrza i Nadwozia",
//             description:
//               "Poczuj luksus od środka i na zewnątrz. Nasz detailing wnętrza gwarantuje nieskazitelną czystość i komfort jazdy.",
//             features: ["Dogłębne czyszczenie", "Pielęgnacja skóry", "Usuwanie nieprzyjemnych zapachów"],
//             path: "/interior-detailing",
//           },
//           {
//             imageSrc: "/images/ceramic-1.jpg",
//             title: "Powłoka Ceramiczna",
//             description:
//               "Chroń swoje auto i podkreśl jego blask dzięki naszej zaawansowanej powłoce ceramicznej, która zabezpiecza lakier.",
//             features: [
//               "Długotrwały połysk",
//               "Odporność na zarysowania",
//               "Łatwość w utrzymaniu",
//             ],
//             path: "/ceramic",
//           },
//         ].map((service, index) => (
//           <ServiceCard key={index} {...service} />
//         ))}
//       </div>
//     </section>
//   </div>
// );

// export default Services;

import React, { useState, useEffect } from "react";
import { Link } from "react-router-dom";

// Komponent animowanej strzałki
const DownArrow = () => (
  <div className="text-center text-gray-500 mb-10">
    <svg
      className="w-8 h-8 mx-auto animate-bounce"
      fill="none"
      stroke="currentColor"
      viewBox="0 0 24 24"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        strokeLinecap="round"
        strokeLinejoin="round"
        strokeWidth="2"
        d="M19 14l-7 7m0 0l-7-7m7 7V3"
      ></path>
    </svg>
  </div>
);

// Karta pojedynczej usługi
const ServiceCard = React.memo(
  ({ imageSrc, title, description, features, path }) => (
    <div className="bg-white rounded-3xl overflow-hidden shadow-lg transition-transform duration-300 ease-in-out lg:mx-8">
      <div className="p-7">
        <img
          src={imageSrc}
          alt={title}
          className="w-full h-64 object-cover rounded-xl shadow-lg"
          loading="lazy"
        />
      </div>
      <div className="p-6">
        <h3 className="text-2xl font-bold mb-2 text-black">{title}</h3>
        <p className="text-gray-600">{description}</p>

        {/* Lista cech (features) z kółkami */}
        <ul className="mt-4 text-gray-700">
          {features.map((feature, index) => (
            <li key={index} className="flex items-center mb-2">
              <svg
                className="w-4 h-4 mr-2 text-custom-blue"
                fill="currentColor"
                viewBox="0 0 20 20"
                xmlns="http://www.w3.org/2000/svg"
              >
                <circle cx="10" cy="10" r="8" />  {/* Kółko */}
              </svg>
              {feature}
            </li>
          ))}
        </ul>

        <Link
          to={path}
          className="mt-4 relative inline-flex items-center justify-center p-4 px-6 py-3 overflow-hidden font-medium text-indigo-600 transition duration-300 ease-out border-2 border-custom-blue rounded-full shadow-md group"
        >
          <span className="absolute inset-0 flex items-center justify-center w-full h-full text-white duration-300 -translate-x-full bg-custom-blue group-hover:translate-x-0 ease">
            <svg
              className="w-6 h-6"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                strokeWidth="2"
                d="M14 5l7 7m0 0l-7 7m7-7H3"
              ></path>
            </svg>
          </span>
          <span className="absolute flex items-center justify-center w-full h-full text-custom-blue transition-all duration-300 transform group-hover:translate-x-full ease">
            Learn More
          </span>
          <span className="relative invisible">Learn More</span>
        </Link>
      </div>
    </div>
  )
);

// Sekcja usług (pobierane dynamicznie)
const Services = () => {
  const [services, setServices] = useState([]);

  useEffect(() => {
    fetch("https://localhost:7045/api/service")  // Dynamiczne pobieranie danych
      .then((res) => res.json())
      .then((data) => {
        setServices(data);
      })
      .catch((err) => console.error("Błąd ładowania usług:", err));
  }, []);

  return (
    <div className="bg-gradient-to-b from-custom-blue to-white p-6 lg:p-10">
      <section id="services" className="container mx-auto my-12">
        <h1 className="pl-2 text-center text-3xl font-bold border-custom-blue md:text-5xl">
          Nasze Usługi
        </h1>
        <p className="text-lg text-gray-500 text-center p-5">
          Podnieś standard swojego auta dzięki naszym profesjonalnym usługom detailingu.
        </p>
        
        <DownArrow />

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 mt-8">
          {services.length > 0 ? (
            services.map((service, index) => (
              <ServiceCard key={index} {...service} />
            ))
          ) : (
            <p className="text-center text-gray-400 col-span-3">
              Brak dostępnych usług.
            </p>
          )}
        </div>
      </section>
    </div>
  );
};

export default Services;

